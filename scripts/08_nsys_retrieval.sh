#!/usr/bin/env bash
# Host nsys profile of 8-GPU MovieLens-20M retrieval (cudaProfilerApi window).
set -euo pipefail
...
TS="$(timestamp)"
HOST="$(hostname -s)"
OUT="${PROFILE_ROOT}/nsys/retrieval_${TS}_${HOST}"
LOG="${LOG_ROOT}/08_nsys_retrieval_${TS}.log"
...

{
  ...
  "${NSYS_BIN}" profile \
    ...
    -- "${SCRIPT_DIR}/docker_run.sh" --privileged --name "${CONTAINER_NAME}-nsys-retr" -- \
      bash -lc "
        set -euo pipefail
